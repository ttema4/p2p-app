...
Chains Analysis::analyze(Orders &orders, MarketRates &market_rates) {
    ...
    for (const std::string &current_market : markets) {
        ...
        if (ORDERS_FOR_BUY_LOGS_ON) {
            log_file << "Market: " << current_market << std::endl;
            log_file << "orders_for_buy size: " << orders_for_buy.list.size() << std::endl;
            for (int i = 0; i < 5; i++) {
                log_file << i << ". We buy: " << orders_for_buy.list[i].coin2 << " for " << orders_for_buy.list[i].coin1
                         << " by price " << orders_for_buy.list[i].exchange_rate << std::endl;
            }
        }

        if (ORDERS_FOR_SELL_LOGS_ON) {
            log_file << "Market: " << current_market << std::endl;
            log_file << "orders_for_sell size: " << orders_for_sell.list.size() << std::endl;
            for (int i = 0; i < orders_for_sell.list.size(); i++) {
                log_file << i << ". We sell: " << orders_for_sell.list[i].coin2 << " for "
                         << orders_for_sell.list[i].coin1 << " by price " << orders_for_sell.list[i].exchange_rate
                         << std::endl;
            }
        }
        ...
        if (SELL_BY_COIN_LOGS_ON) {
            log_file << "Market: " << current_market << std::endl;
            log_file << "sell_by_coin size: " << sell_by_coin.size() << std::endl;
            log_file << "sell_by_coin:" << std::endl;
            for (auto it = sell_by_coin.begin(); it != sell_by_coin.end(); ++it) {
                log_file << it->first << std::endl;
            }
        }

        if (MARKET_RATES_LOGS_ON) {
            log_file << "Market: " << current_market << std::endl;
            log_file << "market_rates:" << std::endl;
            for (auto it = market_rates.list[current_market].begin(); it != market_rates.list[current_market].end();
                 ++it) {
                for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2) {
                    log_file << it->first << " " << it2->first << " " << it2->second << std::endl;
                }
            }
        }
