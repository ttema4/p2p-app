...
void Analysis::analyze(Chains &chains, Orders &orders,
    ...
    if (ORDERS_FOR_BUY_LOGS_ON) {
        std::cout << "orders_for_buy size: " << orders_for_buy.list.size()
                  << std::endl;
        for (int i = 0; i < 5; i++) {
            std::cout << i << ". Покупаем: " << orders_for_buy.list[i].coin2
                      << " за " << orders_for_buy.list[i].coin1 << " по цене "
    ...
    if (ORDERS_FOR_SELL_LOGS_ON) {
        ...
        for (int i = 0; i < orders_for_sell.list.size(); i++) {
            std::cout << i << ". Продаём: " << orders_for_sell.list[i].coin2
                      << " за " << orders_for_sell.list[i].coin1 << " по цене "
    ...
    if (MARKET_RATES_LOGS_ON) {
        ...
        for (auto it = market_rates.list.begin(); it != market_rates.list.end();
             ++it) {
